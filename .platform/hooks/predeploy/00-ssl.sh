...
CRT_VALUE=$(aws ssm get-parameter --name "$CRT_PARAM" --with-decryption --query "Parameter.Value" --output text --region "${AWS_REGION:-eu-north-1}")
KEY_VALUE=$(aws ssm get-parameter --name "$KEY_PARAM" --with-decryption --query "Parameter.Value" --output text --region "${AWS_REGION:-eu-north-1}")
